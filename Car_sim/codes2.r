...
post.samp2<- function(lnlike,y, yName, Name, X, Z, W, A, sig_beta, sig_eta, sig_gam, sig, T){
  ...
  w1=1/exp(w0-min(w0)); 
  ysim0=ysim[,sample(T,1,prob=w1/sum(w1))]
  
  return(list(Mx.b=Mx.b,Mx.eta=Mx.eta,Mx.gam=Mx.gam,ysim=ysim,ysim0=ysim0))
}
##########################################################

ISestimation <- function(y,X,Z,W,T,M,hyper_condC,hyper_MC){
  ...
  #Importance sampling 
  ind=sample(c(5e3:T),M,replace=TRUE)
  ...
  l=0; lam=cover=ksd=rep(0,M); ysim_mx=matrix(,M,dim(y)[1]); 
  ...
  for(j in 1:length(Amx_n)){ 
    ...
    ps=post.samp2(lnlike,y, yName, Name, X, Z, W, Aj, sig_beta=Csig[i,2],sig_eta=Csig[i,1],sig_gam=Csig[i,3], sig, T=30)
    ysim=matrix(ps$ysim0,ncol=1);
    
    result0=MCMC_condC(CondML_flevel_Model3, flevelM3, ysim, yName, Name,X, Z, W, 6e3, hyper_condC) 
    cred=hpdpartition_new(mg2gm,gm2mg,result0$outS[1e3:6e3,],p=0.95,Name)
    cover[l]=Acoverage(paste(Aj,collapse="-"),cred$hpdnames)
    ksd[l]=max(KS.dist(cred0$hpdnames,cred0$hpd,cred$hpdnames,cred$hpd))
  ...
  lam0=1/exp(lam)
  return(list(cover=cover, lam0=lam0, ksd=ksd))
}
...
MCestimation <- function(y,X,Z,W,T=1e4,M=10,hyper_condC,hyper_MC){
  ...
  l=0; 
  for(j in 1:length(Amx_n)){ print(j)
    Aj=as.numeric(unlist(strsplit(Amx_name[j],"-",fixed=TRUE))); nvj=Amx_n[j];
