...
#Exact coverage estimation
exact=MCestimation(y,X,Z,W,T=1e4,M=1e3,hyper_condC,hyper_MC)
exact_coverage=mean(exact$cover)

#Coverage estimation using an improtance sampling
est=ISestimation(y,X,Z,W,T=1e4,M=1e3,hyper_condC,hyper_MC)
#est$lam0 : 1/approx likelihood
#est$cover : coverage indicator
#est$ysim_mx : simulated data
#est$Smx : approximate posterior simulations


##########################################################
#Figure 6 implementation
load('carsim_result.RData')

ks0=seq(from=0.15,to=1,length.out=10); #vector of KS distances
...
exact0=0.98715; sd0=sqrt(exact0*(1-exact0))
dev.new(); 
plot(ks0,cp,xlab='Threshold',ylim=c(0.4,1),ylab='Coverage probability',cex.axis=1.5,cex.lab=1.5); 
arrows(ks0, cp-errks, ks0, cp+errks, length=0.05, angle=90, code=3); 
